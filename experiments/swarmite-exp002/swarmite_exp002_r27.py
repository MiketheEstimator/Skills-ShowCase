...
def make_world(seed):
    w=b.gen_world(seed); r=b.rng_for('r27-effect-type',seed); types={}
    for k,(u,v) in enumerate(b.EDGES):
        if (w.dag_mask>>k)&1: types[(u,v)]=int(r.integers(0,3))
    return w,types

def contrib(kind,w,x):
    if kind==0: return w*x
    if kind==1: return 1.5*w*math.tanh(x)
    return 0.5*w*(x*x-1.0)

def env_sample_nl(w,types,r,n,target=None,setpoint=None):
    X=np.zeros((n,b.N))
    for i in range(n):
        for v in w.order:
            if target==v: X[i,v]=setpoint
                ...
                val=float(r.normal())
                for u in range(b.N):
                    if w.W[u,v]!=0: val+=contrib(types[(u,v)],float(w.W[u,v]),float(X[i,u]))
                X[i,v]=val
    return X

def run_amp(seed,mult):
    w,types=make_world(seed); data=env_sample_nl(w,types,b.rng_for('r27','obs',seed),b.OBS_N); targets=[None]*b.OBS_N; fs,models=b.build_family_models(data,targets); p=b.posterior_from_fs(fs); spend=0;sims=0
    while True:
        ...
        row=env_sample_nl(w,types,b.rng_for('r27','env',seed,step,t,s),1,t,s)[0]; data=np.vstack([data,row]);targets.append(t);spend+=int(b.COSTS[t]);fs,models=b.build_family_models(data,targets);p=b.posterior_from_fs(fs)
