...
import swarmite_exp002_s56_continuous_variance_process as s56

REGIMES=('linear','heteroskedastic')
GAMMAS=s56.GAMMAS.copy()
PRIOR_SD=1.0
NU=4.0
ADJ_CLIP=12.0
EPS=1e-8
LOG_SCALE_OFFSETS=np.linspace(-2.0,2.0,25)
...
def robust_continuous_posterior(data,targets):
    fs,models=b.build_family_models(data,targets); cs=fs.copy(); adjustments=[]; finite=True
    logprior=-0.5*(GAMMAS/PRIOR_SD)**2; logprior-=logsumexp(logprior)
    zero_idx=int(np.argmin(np.abs(GAMMAS)))
    for v in range(b.N):
        keep=np.asarray([t!=v for t in targets],bool); y=np.asarray(data[keep,v],float); n=len(y)
        if n<8: continue
        for pm in range(1<<b.N):
            ...
            z=np.log1p(np.abs(pred)); z=(z-z.mean())/(z.std()+1e-8); center=robust_center_scale(res)
            gl=[]
            for g in GAMMAS:
                fac=np.exp(np.clip(0.5*g*z,-4,4)); vals=[]
                for off in LOG_SCALE_OFFSETS:
                    s=center*math.exp(float(off)); vals.append(t_loglik(res,s*fac))
                gl.append(float(max(vals)))
            gl=np.asarray(gl,float); base=float(gl[zero_idx]); marginal=logsumexp(gl+logprior)
            adj=float(np.clip(marginal-base,-ADJ_CLIP,ADJ_CLIP)); cs[v,pm]=fs[v,pm]+adj; adjustments.append(adj); finite=finite and math.isfinite(adj)
...
def world_row(external_seed):
    ...
    p57,ok,diag=robust_continuous_posterior(data,targets)
    bm=b.posterior_metrics(np.asarray(base['p0'],float),base['true_mask']); cm=b.posterior_metrics(p57,base['true_mask'])
    out=dict(base); out.update({'p57':p57.tolist(),'p57_edge_delta_vs_baseline':float(cm['edge_error']-bm['edge_error']),'p57_brier_delta_vs_baseline':float(cm['brier']-bm['brier']),'robust_process_diag':diag,'p57_mechanics_ok':ok}); return out
...
def mechanics(rows,n_each):
    return all(sum(r['regime']==rg for r in rows)==n_each for rg in REGIMES) and all(r['spend']<=15 and r['trace_identical'] and r['finite'] and r['mechanics_ok'] and r['p57_mechanics_ok'] for r in rows)


def boot(x,reps=10000,seed=25757):
...
def evaluate(rows,seed):
    ...
    for r in rows:
        op=bool(r['outer_promote']); c=float(r['s30_edge_delta_vs_baseline']) if op else 0.0; x=float(r['p57_edge_delta_vs_baseline']) if op else 0.0
        cb=float(r['s30_brier_delta_vs_baseline']) if op else 0.0; xb=float(r['p57_brier_delta_vs_baseline']) if op else 0.0
        control.append(c); cand.append(x); control_b.append(cb); cand_b.append(xb); outer.append(op); regs.append(r['regime']); madj.append(r['robust_process_diag']['mean_abs_adjustment']); frac.append(r['robust_process_diag']['fraction_abs_adjustment_ge_002'])
        harms_c += int(op and r['s30_edge_delta_vs_baseline']>.50); harms_x += int(op and r['p57_edge_delta_vs_baseline']>.50)
...
if __name__=='__main__':
    me=generate(88501,2); out={'mechanics':{'passed':mechanics(me,2)}}
    if not out['mechanics']['passed']: out['disposition']='BLOCKED_MECHANICS'
    else:
        tr=generate(88601,64); ev=evaluate(tr,25757); out['training']=ev
        if not qualifies(ev): out['disposition']='FALSIFIED_AT_TRAINING'
        else:
            va=generate(89001,32); vv=evaluate(va,25758); out['validation']=vv
            if not qualifies(vv,validation=True): out['disposition']='FALSIFIED_ON_VALIDATION'
            else:
                co=generate(89501,64); cv=evaluate(co,25759); out['confirmation']=cv; out['disposition']='SUPPORTED' if qualifies(cv,validation=True,confirmation=True) else 'FALSIFIED_ON_CONFIRMATION'
